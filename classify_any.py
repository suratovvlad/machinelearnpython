# -*- coding: utf-8 -*-
"""
Created on Thu Jun 11 15:18:42 2015
...
def relevantDocs(doc_str):
	...
	return top10_toReturn
	
	
from os import getenv
import pymssql

def getFromMSSQL(doc_ids):
...
class ShowPatentsDialog(QtGui.QDialog):
	def __init__(self):
		super(ShowPatentsDialog, self).__init__()
		self.textEdit = QtGui.QTextEdit()
		self.textEdit.setMinimumSize(QtCore.QSize(400, 400*0.75))
	...
	def showMyInfo(self, list_of_docs):
		
		string = ""
		
		for doc__ in list_of_docs:
			string += "=" * 80
			string += "\n"
			string += "PLACE:\t{0}\n".format(doc__['place'])
			string += "AUTHOR:\t{0}\n".format(doc__['author'])
...
class MainWindow(QtGui.QMainWindow):
	def __init__(self):
		...
		self.createActions()
		self.createMenus()
		self.createQueryTextEdit()
		
	def createQueryTextEdit(self):
		self.textEdit = QtGui.QTextEdit()
		self.setCentralWidget(self.textEdit)
	
	def search(self):
		ids = relevantDocs(self.textEdit.toPlainText())
		dial = ShowPatentsDialog()
		dial.showMyInfo(getFromMSSQL(ids))
		dial.exec()
		#QtGui.QMessageBox.about(self, "IDS", "{0}".format())
	
	def createActions(self):
		self.getDocuments = QtGui.QAction("&Search...",
                self, statusTip="Search for documents", triggered=self.search)
	...
	def createMenus(self):
		...
		self.fileMenu.addAction(self.getDocuments)
